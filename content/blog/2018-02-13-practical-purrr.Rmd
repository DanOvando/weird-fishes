...
1. Using `purrr` as a replacement for loops/ `apply`

2. Using `purrr` to wrangle lists

3. Using `purrr` for data analysis

.Rmd can be found [here](https://github.com/DanOvando/weird-fishes/blob/master/content/blog/2018-02-13-practical-purrr.Rmd)
...
For other problems, things might be a little cleaner if you used something like `purrr`, but the amount of time required to set it up might not be worth it, so maybe a loop is the way to go. 

In my opinion, the reason to move away from loops is when doing so really helps you solve a problem, not when it just makes your code look fancier. 

To work backwards then, `apply`, and `map` basically take on the tasks of a `for` loop
...
I **strongly** recommend you avoid relying on global variables inside functions: it can very easily cause unintended and sneaky behavior (I've been burned before). 
...
One annoying thing about using `map` (or `apply`) in place of loops is that it can make debugging much harder to deal with. With a loop, it can be a lot easier to see where exactly an error occurred and your loop failed (e.g. look at the index of the loop when the error occurred). With `map`, it can be much harder to figure out where the problem is, especially if you have a very large object. 
