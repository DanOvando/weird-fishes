...
$$[\alpha,h,\beta,\sigma | r] \propto normal( log(r) | bh(h,\alpha),\sigma) * normal(log(\beta)|10,2) * unif(h|0.2,1) * normal(\alpha|10*max(r),0.1*max(r)) * cauchy(\sigma|0,2.5)$$
...
bh_fit <- stan(
  ...
  init = list(
    ...
    list(
      h = 0.21,
      alpha = 3 * data$max_r,
      log_beta = log(3 *max(sal_data$ssb))
    ),
    list(
      h = 0.8,
      alpha = 1 * data$max_r,
